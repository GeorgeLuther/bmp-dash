...
CREATE OR REPLACE FUNCTION "public"."has_any_role"("p_labels" "text"[]) RETURNS boolean
...
select exists (
  select 1
  from latest l
  join target t on t.role_id = l.role_id
...
ALTER FUNCTION "public"."has_any_role"("p_labels" "text"[]) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."has_role"("p_label" "text") RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
with target as (
...
ALTER FUNCTION "public"."has_role"("p_label" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."is_system_admin"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$ select public.has_role('System Administrator'); $$;
...
CREATE OR REPLACE FUNCTION "public"."is_top_management"() RETURNS boolean
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO ''
    AS $$ select public.has_role('Top Management'); $$;
...
ALTER TABLE "public"."user_table_views" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "public"."v_current_user_context" WITH ("security_invoker"='on') AS
 WITH "user_roles" AS (
         SELECT "r"."role_id",
            ...
            "r"."proficiency_label",
            "r"."assigned_since"
           FROM "public"."v_current_user_active_roles" "r"
        ), "user_departments" AS (
         SELECT DISTINCT "d"."id" AS "department_id",
            "d"."label" AS "department_label"
           FROM (("user_roles" "ur"
             JOIN "public"."departments_roles" "dr" ON (("dr"."role_id" = "ur"."role_id")))
             JOIN "public"."departments" "d" ON (("d"."id" = "dr"."department_id")))
        )
 SELECT "p"."id" AS "personnel_id",
    ...
    COALESCE(( SELECT "json_agg"("json_build_object"('role_id', "ur"."role_id", 'role_label', "ur"."role_label", 'involvement_id', "ur"."involvement_id", 'involvement_label', "ur"."involvement_label", 'proficiency_id', "ur"."proficiency_id", 'proficiency_label', "ur"."proficiency_label") ORDER BY "ur"."role_label") AS "json_agg"
           FROM "user_roles" "ur"), '[]'::"json") AS "roles",
    COALESCE(( SELECT "json_agg"("json_build_object"('department_id', "ud"."department_id", 'department_label', "ud"."department_label") ORDER BY "ud"."department_label") AS "json_agg"
           FROM "user_departments" "ud"), '[]'::"json") AS "departments",
    (EXISTS ( SELECT 1
           FROM "user_roles"
          WHERE ("user_roles"."role_label" = 'System Administrator'::"text"))) AS "is_system_admin",
    (EXISTS ( SELECT 1
           FROM "user_roles"
          WHERE ("user_roles"."role_label" = 'Top Management'::"text"))) AS "is_top_management"
   FROM "public"."personnel" "p"
  WHERE ("p"."id" = "auth"."uid"());
...
ALTER TABLE ONLY "public"."user_table_views"
    ADD CONSTRAINT "user_table_views_created_by_table_id_name_key" UNIQUE ("created_by", "table_id", "name");



ALTER TABLE ONLY "public"."user_table_views"
    ADD CONSTRAINT "user_table_views_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "utv_creator_table" ON "public"."user_table_views" USING "btree" ("created_by", "table_id");



CREATE UNIQUE INDEX "utv_global_unique" ON "public"."user_table_views" USING "btree" ("table_id", "name") WHERE ("is_global" = true);



CREATE INDEX "utv_table" ON "public"."user_table_views" USING "btree" ("table_id");



CREATE OR REPLACE TRIGGER "handle_updated_at" BEFORE UPDATE ON "public"."user_table_views" FOR EACH ROW EXECUTE FUNCTION "extensions"."moddatetime"('updated_at');
...
ALTER TABLE ONLY "public"."user_table_views"
    ADD CONSTRAINT "user_table_views_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE CASCADE;
...
CREATE POLICY "utv_admin_manage_all" ON "public"."user_table_views" TO "authenticated" USING ("public"."is_system_admin"()) WITH CHECK ("public"."is_system_admin"());



CREATE POLICY "utv_owner_manage_unlocked" ON "public"."user_table_views" TO "authenticated" USING ((("created_by" = "auth"."uid"()) AND ("is_locked" = false))) WITH CHECK ((("created_by" = "auth"."uid"()) AND ("is_locked" = false)));



CREATE POLICY "utv_owner_select" ON "public"."user_table_views" FOR SELECT TO "authenticated" USING (("created_by" = "auth"."uid"()));



CREATE POLICY "utv_read_global" ON "public"."user_table_views" FOR SELECT TO "authenticated" USING (("is_global" = true));
...
REVOKE ALL ON FUNCTION "public"."has_any_role"("p_labels" "text"[]) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."has_any_role"("p_labels" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."has_any_role"("p_labels" "text"[]) TO "service_role";



REVOKE ALL ON FUNCTION "public"."has_role"("p_label" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."has_role"("p_label" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."has_role"("p_label" "text") TO "service_role";
...
GRANT ALL ON TABLE "public"."user_table_views" TO "authenticated";
GRANT ALL ON TABLE "public"."user_table_views" TO "service_role";
